...
public class Goods_in_supplyDAO_Implementation extends CommonDAOImplementation<Goods_in_supply, Long> implements Goods_in_supplyDAO {
    ...
    @Override
    public List<Goods_in_supply> getAllGoods_in_supply(Long id) {
        try (Session session = sessionFactory.openSession()) {
            Query<Goods_in_supply> query = session.createQuery("FROM Goods_in_supply " +
                    "WHERE supply = :id", Goods_in_supply.class)
                    .setParameter("id", id);
