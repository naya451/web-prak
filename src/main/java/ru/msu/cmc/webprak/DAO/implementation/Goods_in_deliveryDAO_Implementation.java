...
public class Goods_in_deliveryDAO_Implementation extends CommonDAOImplementation<Goods_in_delivery, Long> implements Goods_in_deliveryDAO {
    ...
    @Override
    public List<Goods_in_delivery> getAllGoods_in_delivery(Long id) {
        try (Session session = sessionFactory.openSession()) {
            Query<Goods_in_delivery> query = session.createQuery("FROM Goods_in_delivery " +
                            "WHERE delivery = :id", Goods_in_delivery.class)
                    .setParameter("id", id);
